...
| Directory links | Montserrat 400, 11px/1.4, uppercase, 2px tracking |
...
Montserrat and Raleway were identified by matching the screenshot geometry against font metrics; the supplied HTML does not contain the compiled `@font-face` rules. The portable head fragment loads the required 400 weights from Google Fonts. A privacy-restricted deployment may self-host licensed copies and retain the same family names.
